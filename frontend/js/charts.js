...
import { fmtBig, fmtPrice, fmtPct, fmtRatio, fmtNum } from "./format.js?v=79";
...
export function renderAllCharts(data) {
  ...
  chartDividends(data);
  chartMos(data);
  renderQualityScorecardCharts(data);
...
/* ═════════════════════════════════════════════════════════════════════
   GRÁFICOS DE CALIDAD & SOLIDEZ BUFFETT (ESTILO MÚLTIPLOS HISTÓRICOS)
   ═════════════════════════════════════════════════════════════════════ */
export function renderQualityScorecardCharts(data) {
  if (!data || !data.annuals || !data.annuals.length) return;
  const a = data.annuals;
  const cc = getChartColors();
  const ba = baseAxisStyle(cc);
  const cur = data.profile?.currency || 'USD';
  const sc = data.scorecard || {};
  const chkMap = {};
  (sc.checks || []).forEach(c => { chkMap[c.id] = c; });

  const setBadge = (id, passed, text) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.className = 'stat-badge ' + (passed === true ? 'pass' : passed === false ? 'fail' : 'na');
    el.textContent = text || (passed === true ? '✓ Pasa' : passed === false ? '✗ No cumple' : '—');
  };

  const setStatChips = (statId, chips) => {
    const el = document.getElementById(statId);
    if (!el) return;
    el.innerHTML = chips.map(([c, html]) =>
      `<span class="stat-chip"><span class="dot" style="background:${c}"></span>${html}</span>`).join('');
  };
